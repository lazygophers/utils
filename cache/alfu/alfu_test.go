...
func TestNewWithConfig(t *testing.T) {
	cache, err := NewWithConfig[string, int](3, 0.8, 10*time.Minute)
	if err != nil {
		t.Fatalf("Failed to create cache: %v", err)
	}
	stats := cache.Stats()
...
func TestNewWithConfigError(t *testing.T) {
	_, err := NewWithConfig[string, int](3, 1.5, time.Minute)
	if err == nil {
		t.Error("Expected error for invalid decay factor")
	}
}
...
func TestDecayFunctionality(t *testing.T) {
	...
	cache, err := NewWithConfig[string, int](3, 0.5, 10*time.Millisecond)
	if err != nil {
		t.Fatalf("Failed to create cache: %v", err)
	}
...
func TestTimeBasedDecay(t *testing.T) {
	cache, err := NewWithConfig[string, int](3, 0.8, 5*time.Millisecond)
	if err != nil {
		t.Fatalf("Failed to create cache: %v", err)
	}
...
func TestNewWithConfigZeroCapacityError(t *testing.T) {
	// Test the capacity <= 0 error in NewWithConfig
	_, err := NewWithConfig[string, int](0, 0.9, 5*time.Minute)
	if err == nil {
		t.Error("Expected error for zero capacity in NewWithConfig")
	}
}
