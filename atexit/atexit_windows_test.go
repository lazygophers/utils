...
func TestWindowsSignalHandling(t *testing.T) {
	// Windows 特定的信号处理测试
	if os.Getenv("BE_WINDOWS_SIGNAL_TEST") == "1" {
		Register(func() {
			os.Stdout.Write([]byte("windows_callback_executed"))
			os.Stdout.Sync()
		})
	...
	cmd := exec.Command(os.Args[0], "-test.run=TestWindowsSignalHandling")
	cmd.Env = append(os.Environ(), "BE_WINDOWS_SIGNAL_TEST=1")
	...
	// Windows 使用 os.Interrupt 信号
	err = cmd.Process.Signal(os.Interrupt)
	if err != nil {
		// Windows 信号处理可能不同，跳过测试
		t.Skip("Windows 信号处理测试跳过")
		return
	}
	...
	case <-done:
		...
		if !strings.Contains(outputStr, "windows_callback_executed") {
			t.Logf("Windows信号处理输出: %s", outputStr)
			// Windows 信号处理行为可能不同，这不算错误
		}
	case <-time.After(2 * time.Second):
		cmd.Process.Kill()
		t.Skip("Windows 信号处理测试超时，跳过")
	}
}
